...
- KYC (`/kyc`): People directory derived from transaction counterparties (names). Background page shows a transaction-derived summary (inflows/outflows, top counterparties, currencies, channels, sanctions breakdown).
...
- Entities/KYC: `GET /api/entities`, `GET /api/entities/{id}`.
